...
import { SUBJECT_COLORS, SUBJECT_ICONS, Subject, AssignmentType } from '../../types';
...
const GRADE_COMPONENTS = [
  { key: 'performanceTask' as const, label: 'Performance Task', icon: '🏗️' },
  { key: 'writtenWorks'   as const, label: 'Written Works',    icon: '📝' },
  { key: 'exam'           as const, label: 'Exam',             icon: '📋' },
] as const;

type GradeComponentKey = typeof GRADE_COMPONENTS[number]['key'];

type TypeWeights = Partial<Record<GradeComponentKey, number>>;

const DEFAULT_TYPE_WEIGHTS: TypeWeights = {
  performanceTask: 40,
  writtenWorks: 30,
  exam: 30,
};
...
export function SubjectForm({ isOpen, onClose, onSubmit, editingSubject }: SubjectFormProps) {
  ...
  const [typeWeights, setTypeWeights] = useState<TypeWeights>(
    (editingSubject?.typeWeights as TypeWeights) || DEFAULT_TYPE_WEIGHTS
  );
  ...
  React.useEffect(() => {
    if (editingSubject) {
      ...
      setTypeWeights((editingSubject.typeWeights as TypeWeights) || DEFAULT_TYPE_WEIGHTS);
  ...
  const handleTypeWeightChange = (key: GradeComponentKey, value: number) => {
    setTypeWeights(prev => ({ ...prev, [key]: Math.max(0, Math.min(100, value)) }));
  };
        ...
        <div className="space-y-3">
          ...
          {useTypeWeights && (
            <div className="space-y-3 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
              <p className="text-xs text-gray-500">Set how much each component contributes. Must total 100%.</p>
              {GRADE_COMPONENTS.map(({ key, label, icon: ic }) => (
                <div key={key} className="flex items-center gap-3">
                  <span className="text-base w-6">{ic}</span>
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300 w-28">{label}</span>
                  <input
                    type="range" min={0} max={100} step={5}
                    value={typeWeights[key] ?? 0}
                    onChange={e => handleTypeWeightChange(key, Number(e.target.value))}
                  ...
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      value={typeWeights[key] ?? 0}
                      onChange={e => handleTypeWeightChange(key, Number(e.target.value))}
                      className="input py-1 px-2 text-center text-xs w-14"
                    />
              ...
              <div className={cn(
                ...
                totalTypeWeight === 100 ? 'border-green-200 dark:border-green-500/20' : 'border-red-200 dark:border-red-500/20'
              )}>
                <span className="text-xs font-semibold text-gray-600 dark:text-gray-400">Total</span>
                <span className={cn('text-sm font-bold', totalTypeWeight === 100 ? 'text-green-600 dark:text-green-400' : 'text-red-500')}>
            ...
            <div className="space-y-2 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
              ...
              <div className={cn(
                ...
                <span className={cn(
                  ...
                  {totalTypeWeight}% {totalTypeWeight === 100 ? '✓' : `(need ${100 - totalTypeWeight}% more)`}
                </span>
              </div>
            </div>
          )}
